...
func look(state : Dictionary, neighbours : Array[Drone]) -> Array:
	...
	var visible := neighbours.filter(func(x): return x.state["active"] and _flat_dist_sq(x.state["position"], state["position"]) <= Dmax*Dmax + 0.001)
...
func compute(state : Dictionary, obs : Array, base_pos : Vector3) -> ExecReturn:
	...
	# Max vision filter
	var balabonski_dmax_filter = func(x): return _flat_dist_sq(state["position"], x["position"]) <= 49*D*D+0.001
	
	# id filter
	var lower_id_filter = func(x): return x["id"] < id
	var higher_id_filter = func(x): return x["id"] > id
	...
	var returning = not self_layer.all(func(x): return not x["returning"]) or not \
								...
								or (not self_layer.any(balabonski_dmax_filter))
	...
	if self_layer_collision or layer1.any(balabonski_dmax_filter) or (returning and self_layer.filter(lower_id_filter).any(balabonski_dmax_filter)):
	...
	if returning:
		...
		var con_to_base_while_returning = _flat_dist_sq(pos, base_pos) <= Dmax*Dmax and (pos.y - base_pos.y) <= depth+0.001
		# Already near base !
		if con_to_base_while_returning:
			# Base capture drone
			if(_flat_dist_sq(pos, base_pos) < D*D):
				new_state["KILL"] = true
				return ExecReturn.new(false, "", new_state)
	...
	# Now we are in the connexion layer, so use 7D as max vision as Balabonski et al. intended
	self_layer = self_layer.filter(balabonski_dmax_filter)
	var leaf_drone = self_layer.all(func(x): return  id > x["id"])
	...
	var con_to_base = _flat_dist_sq(pos, base_pos) < balabonski_dmax*balabonski_dmax and  (pos.y - base_pos.y) <= depth+0.001
...
func get_vision_meshes() -> Array[MeshInstance3D]:
	...
	var dmaxR = MeshCreator.create_cylinder(8*D, 2*D, Color.MAGENTA, 32, Vector3(0, -D, 0))
	return [d,dc,dp,dmax, dmaxR]
